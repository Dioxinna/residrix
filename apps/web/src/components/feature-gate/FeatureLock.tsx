...
export function FeatureLock({ feature, description }: { feature: FeatureKey; description?: string }) {
  ...
  return (
    <div className="max-w-2xl mx-auto px-6 py-20 text-center">
      <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-brand/10 border border-brand/30 mb-5">
        <Lock className="text-brand" size={26} strokeWidth={1.75} />
      </div>
      <div className="mb-3 flex items-center justify-center gap-2">
        <TierBadge tier={minTier} />
        <span className="text-xs text-ink-faint">o superior</span>
      </div>
      ...
      <Link
        ...
        className="inline-block bg-brand hover:bg-brand-soft text-white px-6 py-2.5 rounded-full font-medium text-sm"
      >
        Ver planes →
      </Link>
    </div>
