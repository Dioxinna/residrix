...
export function StatusSelector({ incidenceId, currentStatus }: Props) {
  ...
  return (
    <select
      ...
      disabled={saving}
      className="w-full bg-zinc-800 border border-zinc-700 text-zinc-300 text-sm rounded-lg px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-indigo-500 disabled:opacity-50"
    >
      {statusOptions.map((o) => (
