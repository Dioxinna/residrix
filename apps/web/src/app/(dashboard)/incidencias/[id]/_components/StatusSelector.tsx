...
export function StatusSelector({ incidenceId, currentStatus }: Props) {
  ...
  return (
    <select
      ...
      disabled={saving}
      className="w-full bg-[color:var(--c-surface)] border border-[color:var(--glass-border)] text-ink-soft text-sm rounded-lg px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-violet-500 disabled:opacity-50"
    >
      {statusOptions.map((o) => (
